...
abstract class Property<T> {
  ...
  Map toMap() {
    return {"type": type.toString(), "property": getMapData()};
  }
...
class DoubleProperty extends Property<double> {
  DoubleProperty({required double data}) : super(data);
...
class EdgeInsertsProperty extends Property<EdgeInsets> {
  EdgeInsertsProperty({required EdgeInsets data}) : super(data);

  EdgeInsertsProperty.fromMap(Map map)
      : super(EdgeInsets.only(
          left: map["left"] ?? 0,
          right: map["right"] ?? 0,
          top: map["top"] ?? 0,
          bottom: map["bottom"] ?? 0,
...
class BoxConstraintsProperty extends Property<BoxConstraints> {
  BoxConstraintsProperty({required BoxConstraints data}) : super(data);

  BoxConstraintsProperty.fromMap(Map map)
